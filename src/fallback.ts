...
import { toDomainCandidate } from "./scene-validation.js";
...
function phaseFor(request: NextRequest, event: SessionEvent | null) {
  ...
  if (request.scene_state.climax_reached) return "closing" as const;
  if (event?.reaction === "too_much") return "recovery" as const;
  if (progress >= 0.82) return "peak" as const;
...
function scoreCandidate(
  ...
  const domain = toDomainCandidate(candidate);
  if (!checkSceneCompatibility(request.scene_state, domain).compatible) {
    return Number.NEGATIVE_INFINITY;
  }
  let score = scoreSceneCandidate(request.scene_state, domain, {
    progress: request.max_cards > 0 ? request.resolved_count / request.max_cards : 0,
    targetIntensity: target,
    targetIntensityIsFinal: true,
    recentVariantGroups: request.candidates
      .filter((item) => request.recently_seen_card_ids.includes(item.id))
      .map((item) => item.variant_group)
      .filter((value): value is string => Boolean(value)),
    recentContinuityGroups: request.recently_seen_groups,
    selectedInventory: [
      ...request.selected_element_slugs.map((slug) => `element:${slug}`),
      ...request.selected_toy_slugs.map((slug) => `toy:${slug}`),
    ],
  });
  score += 20 - Math.abs(candidate.intensity - target) * 4;
  ...
  if (request.player_count === 1) {
    if (candidate.play_scope === "solo") score += 5;
    if (candidate.performer === "self" && candidate.target === "self")
      score += 3;
    if (candidate.reciprocal_action) score -= 20;
  }
  ...
  if (
    candidate.penetration_method === "juguete" &&
  ...
  if (
    request.current_player_sex === "hombre" &&
  ...
  if (
    request.current_player_sex === "mujer" &&
  ...
  if (event?.reaction !== "repeat_style") {
    const repeatedGroup = (request.recently_seen_groups ?? []).filter(
      (group) => group && group === candidate.gm_continuity_group,
    ).length;
    const repeatedAnatomy = (request.recently_seen_anatomy ?? []).filter(
      (anatomy) => anatomy && anatomy === candidate.anatomy_focus,
    ).length;
...
export function chooseFallback(requestInput: NextRequest): ModelDecision {
  // Keep the fallback callable by older clients and unit fixtures while all
  // execution still flows through the same canonical defaults.
  const request = NextRequestSchema.parse(requestInput);
  const event = lastEvent(request);
  const target = targetIntensity(request);
  const phase = phaseFor(request, event);
  const strategy = strategyFor(phase, event);

  const selected = [...request.candidates].sort((a, b) => {
    const difference =
  ...
  const coupleMessages: Record<ModelDecision["phase"], string> = {
  ...
  const soloMessages: Record<ModelDecision["phase"], string> = {
