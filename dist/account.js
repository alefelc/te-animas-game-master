import { z } from "zod";
import { authenticateAccountToken, inviteAccountUser, DirectusRequestError, readAccountProfile, readAccountUser, saveAccountProfile, updateAccountUser, } from "./directus.js";
...
export const SavedGamePreferencesSchema = z.object({
    ...
    filters: z.record(z.string().trim().min(1).max(100), z.union([z.boolean(), z.number().finite().min(-1000).max(1000)])),
...
export async function authenticateAccount(accessToken) {
    const userId = await authenticateAccountToken(accessToken);
    const user = await readAccountUser(userId);
    if (user.status !== "active") {
        throw new DirectusRequestError("La cuenta no está activa.", 403, "ACCOUNT_INACTIVE", "/users/me");
    }
    return { userId, user };
}
export async function readAccountBundle(accessToken) {
    const { userId, user } = await authenticateAccount(accessToken);
    const profile = await readAccountProfile(userId);
    return { user, profile };
}
export async function patchAccount(accessToken, input) {
...
export async function readProfile(accessToken) {
    const { userId } = await authenticateAccount(accessToken);
    return readAccountProfile(userId);
}
export async function putProfile(accessToken, input) {
