...
function normalizeEscalationScore(value) {
    if (value <= 3)
        return Math.max(-2, value);
    return Math.min(3, Math.max(-2, Math.round(value / 3)));
}
const fivePointScore = (defaultValue) => z.coerce
    .number()
    .int()
    .min(0)
    .max(10)
...
export function normalizeSceneRole(value, context = {}) {
    ...
    if (levelOrder >= 7)
        return "closer";
    if (levelOrder >= 6 || (intensity >= 8 && escalation >= 2))
        return "climax";
    if (levelOrder <= 1 || intensity <= 1)
        return "starter";
    if (levelOrder === 2)
        return "bridge";
    return "continuation";
