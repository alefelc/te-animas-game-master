...
import { authenticateAccount, patchAccount, putProfile, readAccountBundle, readProfile, registerAccount } from "./account.js";
...
function diagnosticAuthorized(request: IncomingMessage) {
  ...
  const bearer = authorization?.startsWith("Bearer ")
    ...
    : null;
  return (
    constantTimeEqual(direct, config.diagnosticToken) ||
    constantTimeEqual(bearer, config.diagnosticToken)
  );
}

function constantTimeEqual(left: string | null | undefined, right: string) {
...
function tokenRateKey(token: string) {
...
const server = createServer(async (request, response) => {
  ...
  if (request.method === "GET" && requestUrl.pathname === "/ready") {
    ...
    return json(response, directus.ok ? 200 : 503, {
      ...
      ready: directus.ok,
      version: API_VERSION,
      dependencies: { content: directus.ok },
      request_id: requestId,
    });
  }
  ...
  if (request.method === "GET" && requestUrl.pathname === "/health") {
    return json(response, 200, {
      ok: true,
      service: "te-animas-adaptive-api",
      version: API_VERSION,
      request_id: requestId,
    });
  }
  ...
  if (requestUrl.pathname.startsWith("/v1/account/")) {
    ...
    if (!accountLimiter.allow(`${clientKey(request)}:${tokenRateKey(token)}`)) {
      return json(response, 429, {
  ...
  if (requestUrl.pathname.startsWith("/v1/couples")) {
    ...
    if (!accountLimiter.allow(`${clientKey(request)}:${tokenRateKey(token)}:couple`)) {
      return json(response, 429, {
  ...
  if (request.method === "POST" && requestUrl.pathname === "/diagnostics/ai") {
    ...
    try {
      ...
      return json(response, error instanceof RequestBodyError ? error.status : 400, {
        ok: false,
        code:
        ...
        error:
          error instanceof RequestBodyError
            ? error.message
            : "La solicitud de diagnóstico no es válida.",
        request_id: requestId,
        api_version: API_VERSION,
      });
    }
  ...
  const gameMasterToken = bearerToken(request);
  if (!gameMasterToken) {
    return json(response, 401, {
      error: "Tenés que iniciar sesión para usar el juego.",
      code: "ACCOUNT_UNAUTHORIZED",
      request_id: requestId,
    });
  }

  let gameMasterUserId: string;
  if (!gameMasterAuthLimiter.allow(`game-master-auth:${clientKey(request)}`)) {
    return json(response, 429, {
  ...
  try {
    const account = await authenticateAccount(gameMasterToken);
    gameMasterUserId = account.userId;
  } catch (error) {
    return accountErrorResponse(response, requestId, error);
  }
  ...
  if (
    ...
    !gameMasterAccountLimiter.allow(`game-master:${gameMasterUserId}`)
  ) {
