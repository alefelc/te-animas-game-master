...
describe("vinculación privada de pareja", () => {
  ...
  it("reserva la invitación con una actualización condicional antes de vincular", async () => {
    const code = "private-invite-code-with-enough-entropy";
    const codeHash = createHmac(
      "sha256",
      "test-pepper-that-is-not-public",
    ...
    vi.stubGlobal("fetch", vi.fn(async (input, init) => {
      ...
      if (call.url.pathname === "/users/me") {
        return Response.json({ data: { id: "user-2" } });
      }
      if (call.url.pathname === "/items/pc_couple_invites" && call.method === "GET") {
        return Response.json({ data: [invite] });
      }
      if (
        call.url.pathname === "/items/pc_couple_invites" &&
        call.method === "PATCH"
      ) {
        if (
          call.url.searchParams.get("filter[id][_eq]") !== invite.id ||
          call.url.searchParams.get("filter[status][_eq]") !== "pending"
        ) {
          return Response.json({ data: [] });
        }
        Object.assign(invite, call.body);
        return Response.json({ data: [invite] });
      }
      if (call.url.pathname === "/items/pc_couple_members") {
        if (call.method === "POST") {
          members.push(call.body as Record<string, unknown>);
          return Response.json({ data: call.body });
        }
        const filterUser = call.url.searchParams.get("filter[user][_eq]");
        if (filterUser) {
          return Response.json({
            data: members.filter((member) => member.user === filterUser),
          });
        }
        const filterCouple = call.url.searchParams.get("filter[couple][_eq]");
        return Response.json({
          data: members.filter((member) => member.couple === filterCouple),
        });
      }
      if (call.url.pathname === "/items/pc_couples" && call.method === "POST") {
        couple = call.body as Record<string, unknown>;
        return Response.json({ data: couple });
      }
      if (call.url.pathname.startsWith("/items/pc_couples/")) {
        return Response.json({
          data: {
            ...couple,
            status: "active",
      ...
      if (
        call.url.pathname === `/items/pc_couple_invites/${invite.id}` &&
        call.method === "PATCH"
      ) {
        Object.assign(invite, call.body);
        return Response.json({ data: invite });
      }
      if (call.url.pathname.startsWith("/users/user-")) {
        const id = call.url.pathname.split("/").at(-1);
        return Response.json({
          data: {
            id,
            email: `${id}@example.com`,
            first_name: id === "user-1" ? "Uno" : "Dos",
            last_name: null,
            status: "active",
          },
        });
      ...
      return Response.json(
        { errors: [{ message: `Ruta inesperada: ${call.url.pathname}` }] },
        { status: 404 },
      );
    }));

    const { acceptCoupleInvite } = await import("../src/couples.js");
    const result = await acceptCoupleInvite("player-token", { code });
    ...
    expect(claim?.url.searchParams.get("filter[status][_eq]")).toBe("pending");
    expect(invite.status).toBe("accepted");
    expect(invite.used_by).toBe("user-2");
  ...
  it("revela solo coincidencias positivas y nunca las respuestas privadas", async () => {
    vi.stubGlobal("fetch", vi.fn(async (input, init) => {
      ...
      if (call.url.pathname === "/items/pc_ai_session_events") {
        return Response.json({
          data: [
            { id: "1", couple: "couple-1", user: "user-1", card: "card-negative", reaction: "interested", created_at: "2026-07-20T10:00:00Z" },
            { id: "2", couple: "couple-1", user: "user-2", card: "card-negative", reaction: "no", created_at: "2026-07-20T10:01:00Z" },
            { id: "3", couple: "couple-1", user: "user-1", card: "card-talk", reaction: "maybe", created_at: "2026-07-20T10:02:00Z" },
            { id: "4", couple: "couple-1", user: "user-2", card: "card-talk", reaction: "interested", created_at: "2026-07-20T10:03:00Z" },
            { id: "5", couple: "couple-1", user: "user-1", card: "card-match", reaction: "favorite", created_at: "2026-07-20T10:04:00Z" },
            { id: "6", couple: "couple-1", user: "user-2", card: "card-match", reaction: "repeat", created_at: "2026-07-20T10:05:00Z" },
            { id: "7", couple: "couple-1", user: "user-1", card: "card-pending", reaction: "interested", created_at: "2026-07-20T10:06:00Z" },
          ],
        });
      }
      return Response.json({ errors: [{ message: "Ruta inesperada" }] }, { status: 404 });
  ...
  it("no convierte un fallo real de actualización en una creación duplicada", async () => {
    ...
    vi.stubGlobal("fetch", vi.fn(async (input, init) => {
      ...
      if (call.url.pathname.startsWith("/items/pc_ai_session_events/")) {
  ...
  it("usa únicamente el almacenamiento compacto que instala la plataforma", () => {
    const source = readFileSync("src/couples.ts", "utf8");
    expect(source).toContain("pc_ai_session_events");
    expect(source).toContain('"filter[status][_eq]"');
    expect(source).not.toContain("pc_couple_invite_claims");
    expect(source).not.toContain("pc_couple_card_signals");
    expect(source).not.toContain("pc_couple_sessions");
  });
